...
export default function Curso3Screen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, (docSnap) => {
      ...
      if (data?.progresso?.curso3) {
        setProgresso(data.progresso.curso3);
      }
    });
  ...
  const toggleLicao = async (licao) => {
    ...
    await updateDoc(userRef, {
      [`progresso.curso3.${licao}`]: newValue
    });
  };

  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Curso 3</Text>
